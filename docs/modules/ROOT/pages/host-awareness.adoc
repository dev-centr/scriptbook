...
PlayTime must know â€œwhat computer is this?â€ **before** it asks the Equivalence Engine for commands. If it asked the engine in order to learn the OS, that would be a loop that never starts.
...
| `family`
| Kernel class: `windows`, `linux`, `darwin`, `freebsd`, â€¦

| `distro`
| Product name: `ubuntu`, `fedora`, `windows`, `macos`, â€¦

| `like`
| Cousins (Linux `ID_LIKE`): `debian`, `rhel`, â€¦
...
| `caps.shells`
| Shells on PATH: `nu`, `pwsh`, `bash`, â€¦

| `caps.pms`
| Package managers on PATH: `winget`, `apt`, `nix`, â€¦

| `caps.formats`
| Formats this host can run: `msi`, `deb`, `flatpak`, `winget`, â€¦
|===
...
| `compileTime`
| Use the D compilerâ€™s `version(Windows)` / `version(OSX)` / else.
...
Catalogs get richer â†’ CI bakes a better cascade â†’ PlayTime wakes up on more machines â†’ people write more playbooks â†’ they register more intents and probes â†’ the next bake is richer. Each release is a ratchet.
